...
//	uint8_t NextByte,BufferByte;									// raw unsigned values of naext byte to plat and mix with
	...
	while(PlayItem!=nullptr)
	{
      ...
      // Convert bytes from unsigned int from 0-255 to a signed value from -127-128. 
      // Effectively transforms it to a sine wave that is centered around x axis.
     ...
     // Flatten the curve if it exceeds thresholds (>128 or <-127)
			if(ByteToPlay>128)
    ...
    readySend = false;  // test correct position to slide this in to play once and stop
...
XT_DAC_Audio_Class::XT_DAC_Audio_Class(uint8_t TheDacPin, uint8_t TimerNo,uint16_t PassedBufferSize)
{
	...
	timerAlarmWrite(timer, 20, true);               // will trigger 50,000 times per second,
...
void XT_MusicScore_Class::sendNextNote() {
  // set the readySend flag
  // This method is to be called before fillbuffer in the loop()
  readySend = true;
}
...
uint8_t XT_MusicScore_Class::NextByte()
{
  ...
  // Hijacking pseudocode
  // if (ChangeNoteCounter ==0) {if (readySend) { ... below code... until the return Instrument->NextByte} else return 0
  // Pressing button will let me call from the loop() sendNextNote() from XT_MusicScore_Class
  // But who unsets the readySend flag at the end of the note? And how?
  // The one who unsets the flag should be at the end of instrument or beat, that is when
  // ChangeNoteCounter==0, it should not just start playing a new note
